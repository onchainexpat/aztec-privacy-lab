...
export interface BrowserSandbox {
  ...
  battleshipPvp: BattleshipPvPContract | null
...
export function initBrowserSandbox(
  ...
  const promise = (async (): Promise<BrowserSandbox> => {
    ...
    const battleshipPvpMod = await import('../contracts/BattleshipPvP')
    ...
    if (state.lottery) {
      await wallet.registerContract(
        deserialize(state.lottery.instance),
        lotteryMod.LotteryContractArtifact,
      )
    }
    if (state.attestation) {
      await wallet.registerContract(
        deserialize(state.attestation.instance),
        attestationMod.IdentityAttestationContractArtifact,
      )
    }
    if (state.batchPay) {
      await wallet.registerContract(
    ...
    if (state.battleshipPvp) {
      await wallet.registerContract(
        deserialize(state.battleshipPvp.instance),
        battleshipPvpMod.BattleshipPvPContractArtifact,
      )
    }
    if (state.payroll) {
      await wallet.registerContract(
        deserialize(state.payroll.instance),
        payrollMod.PayrollContractArtifact,
      )
    }
    if (state.rewards) {
      await wallet.registerContract(
        deserialize(state.rewards.instance),
        rewardsMod.RewardsContractArtifact,
      )
    }
    if (state.blackjack) {
      await wallet.registerContract(
