...
export function BatchPayPanel({ state, onClose }: Props) {
  ...
  async function handleInit() {
    ...
    try {
      ...
      if (!sb.batchPay) throw new Error('BatchPay not deployed - re-run sandbox:setup')
      setSandbox(sb)
      // Default both recipients to the admin's own address (self-pay demo).
      // Edit to send to anyone; the privacy property is identical.
      const self = sb.admin.toString()
      setTo1(self)
      setTo2(self)
  ...
  async function handlePayTwo() {
    ...
    try {
      ...
      // The embedded wallet auto-injects the inner authwits for each
      // transfer on the caller's behalf.
      const res = (await sandbox.batchPay.methods
        .pay_two(r1, a1, nonce1, r2, a2, nonce2)
        .send({ from: sandbox.admin })) as unknown as {
  ...
  return (
    <section className="mt-10 rounded-2xl border border-black/10 bg-white p-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold">Batch private payment - nested composability</h3>
        <button onClick={onClose} className="text-sm text-black/50 underline-offset-4 hover:underline">
          Close
        </button>
      </div>
      <p className="mt-2 text-sm text-black/60">
        One private tx pays two recipients. The BatchPay contract makes two nested{' '}
        <code className="font-mono text-xs">Token.transfer_in_private</code> sub-calls inside a
        single private function - a real private call stack across two contracts. The embedded
        wallet auto-signs each inner transfer on your behalf.
      </p>
